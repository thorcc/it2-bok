...
<details>
<summary>Mermaid-kode (kopierbar)</summary>
...
Hver klasse har ansvar for sine egne ting.
...
1. Hvorfor er det lurt å ha en egen `Tweet`-klasse i stedet for bare å lagre tekst?
2. Hvorfor ligger metoden `lag_tweet()` i `Bruker` og ikke i `Twitter`?
3. Hvorfor er det `Twitter` som lager den globale feeden?
